...
const initTime = (self: VanillaCalendar) => {
    ...
    if (time12 || self.settings.selection.time === 24) {
        let userTime = false;
        if (self.settings.selected.time && self.settings.selected.time?.length > 0) {//array
            const regExr = time12 ? /^([0-9]|0[1-9]|1[0-2]):([0-5][0-9])|(AM|PM)/g
                : /^([0-1]?[0-9]|2[0-3]):([0-5][0-9])/g;

            self.settings.selected.time[0].replace(regExr, (_, p1: string, p2: string, p3: string) => {
                if (p1 && p2) {
                    userTime = true;
                    self.selectedHours[0] = p1;
                    self.selectedMinutes[0] = p2;
                }
                if (p3 && time12) {
            ...
            if (self.settings.selected.time.length === 2) {
                self.settings.selected.time[1].replace(regExr, (_, p1: string, p2: string, p3: string) => {
                    if (p1 && p2) {
                        userTime = true;
                        self.selectedHours[1] = p1;
                        self.selectedMinutes[1] = p2;
                    }
                    if (p3 && time12) {
                        self.selectedKeeping = p3;
                    } else if (time12) {
        ...
        if (!userTime && (time12)) {
            self.selectedHours[0] = transformTime12(String(self.date.today.getHours()));
            self.selectedMinutes[0] = String(self.date.today.getMinutes());
            ...
            if (self.settings.selection.day === 'multiple' || self.settings.selection.day === 'multiple-ranged') {
                self.selectedHours[1] = transformTime12(String(self.date.today.getHours()));
                self.selectedMinutes[1] = String(self.date.today.getMinutes());
            }

        } else if (!userTime) {
            self.selectedHours[0] = String(self.date.today.getHours());
            self.selectedMinutes[0] = String(self.date.today.getMinutes());
            if (self.settings.selection.day === 'multiple' || self.settings.selection.day === 'multiple-ranged') {
                self.selectedHours[1] = String(self.date.today.getHours());
                self.selectedMinutes[1] = String(self.date.today.getMinutes());
            }
        }

        self.selectedHours[0] = Number(self.selectedHours[0]) < 10 ? `0${Number(self.selectedHours[0])}` : `${self.selectedHours[0]}`;
        self.selectedMinutes[0] = Number(self.selectedMinutes[0]) < 10 ? `0${Number(self.selectedMinutes[0])}` : `${self.selectedMinutes[0]}`;
        self.selectedTime = [];
        self.selectedTime.push(`${self.selectedHours[0]}:${self.selectedMinutes[0]}${self.selectedKeeping ? ` ${self.selectedKeeping}` : ''}`);
        if (self.settings.selection.day === 'multiple' || self.settings.selection.day === 'multiple-ranged') {
            self.selectedHours[1] = Number(self.selectedHours[1]) < 10 ? `0${Number(self.selectedHours[1])}` : `${self.selectedHours[1]}`;
            self.selectedMinutes[1] = Number(self.selectedMinutes[1]) < 10 ? `0${Number(self.selectedMinutes[1])}` : `${self.selectedMinutes[1]}`;
            self.selectedTime.push(`${self.selectedHours[1]}:${self.selectedMinutes[1]}${self.selectedKeeping ? ` ${self.selectedKeeping}` : ''}`);
        }
